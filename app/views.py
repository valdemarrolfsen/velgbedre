...
def frontpage_view(request, type):
    ...
    if request.method == "POST":
        if 'other_name' in request.POST:
            ...
            Mail_sender.send_wish_mail(request.POST.get('to_email'), request.POST.get('to_name'),
                                       request.POST.get('from_name'))
            email_sent = True
        else:
            sub = Subscriber()
...
def package_view(request):
    ...
    if request.method == "POST":
        Mail_sender.send_wish_mail(request.POST.get('to_email'), request.POST.get('to_name'),
                                   request.POST.get('from_name'), request.POST.get('package_name'))
